...
class QuickBrushGreasePencilPanel(bpy.types.Panel):
    bl_label = "Grease Pencil Settings"
    bl_idname = "VIEW_3D_PT_QUICK_BRUSH_GREASE_PENCIL_LAYOUT"
    bl_parent_id = "VIEW_3D_PT_QUICK_BRUSH_SETTINGS_LAYOUT"
    bl_space_type = 'VIEW_3D'
    bl_region_type = 'UI'
    ...
    def draw(self, context):
        ...
        try_init_collection(my_data.grease_pencil_brush_slots, SLOT_COUNT)

        layout.operator("quick_brush.copy_grease_pencil_brush_slots", text="Copy Grease Pencil Brushes")
        ...
        for i in range(0, SLOT_COUNT):
            ...
            c2.scale_x = 2
            c1.label(text="Slot {x}".format(x=i+1))
            c2.template_ID(my_data.grease_pencil_brush_slots[i], "brush")
